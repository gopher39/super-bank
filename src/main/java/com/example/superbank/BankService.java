...
public class BankService {
    ...
    public BigDecimal addMoney(Long to, BigDecimal ammount) {
            BigDecimal currentBalance = repository.getBalanceForId(to);
            if (currentBalance == null) {
                repository.save(to, ammount);
                return ammount;
            } else {
                BigDecimal updatedBalance = currentBalance.add(ammount);
    ...
    public void makeTransfer(TransferBalance transferBalance) {
        ...
        if (transferBalance.getAmmount().compareTo(fromBalance) > 0) {
            throw new IllegalArgumentException("no money!");
        }

        BigDecimal updatedFromBalance = fromBalance.subtract(transferBalance.getAmmount());
        BigDecimal updatedToBalance = toBalance.add(transferBalance.getAmmount());
